local B = {}
	...
	local function buildLevel(levelNo)
		...
		local slowTrap = require "Traps.slow_trap"
		local deathTrap = require "Traps.fire_trap"
		local winTrap = require "Traps.win_trap"
		local levelName = GLOBAL_levelsPath.."level"..levelNo..".BOOMMAP"
		local map = {}
		...
		b.floor = display.newRect(GLOBAL_ccx, GLOBAL_ccy,512000,512000)
		display.setDefault( "textureWrapX", "repeat" )
		display.setDefault( "textureWrapY", "repeat" )
		b.floor.fill = {type = "image",filename = GLOBAL_backgroundPath.."FloorTile.png"}
		...
		for mapCounter = 1, table.getn(map), 1 do
			if tonumber(map[mapCounter][1]) < 11 and tonumber(map[mapCounter][1]) > 0 then
				b.physlevel[mapCounter] = display.newPolygon(
					...
					getVertices(
						tonumber(map[mapCounter][1]),
						tonumber(map[mapCounter][2])
					)
				)
				b.physlevel[mapCounter].fill = {
					type="image",
					filename=GLOBAL_backgroundPath..objectFileName[tonumber(map[mapCounter][1])]..tonumber(map[mapCounter][2])..".png"
				}
			...
			elseif tonumber(map[mapCounter][1]) < 51 then
				...
				if tonumber(map[mapCounter][1]) == 41 then
					b.slowTraps[#b.slowTraps + 1] = slowTrap.spawn(
						tonumber((map[mapCounter][3])-448)*size,
						tonumber((map[mapCounter][4])-448)*size
					)
				else
					--traps
					b.deathTraps[#b.deathTraps + 1] = deathTrap.spawn(
						tonumber((map[mapCounter][3])-448)*size,
						tonumber((map[mapCounter][4])-448)*size
					)
				end
			...
			elseif tonumber(map[mapCounter][1]) == 100 then
				b.slowTraps[#b.slowTraps + 1] = winTrap.spawn(
					tonumber((map[mapCounter][3])-448)*size,
					tonumber((map[mapCounter][4])-448)*size
				)
