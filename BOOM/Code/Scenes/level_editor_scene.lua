...
function scene:create( event )
	...
	objectFileName = {"LavaTile","LavaTile","WallDiagonal","WallFlat","WinTile"}
	enemyFileName = {"Imp","HellPup","Minator"}
	trapFileName = {"Slow","Death"}
	miscFileName = {}
	...
	selectedShape = display.newPolygon(
		overlay,
		display.contentWidth/16,
		display.contentHeight-100,
		getVertices(selectedObject,rotation)
	)
	selectedShape.fill = { type="image", filename=GLOBAL_graphicsPath.."Background/"..objectFileName[selectedObject].."1.png" }
	...
	buttonText = display.newText(overlay,"MAIN MENU", 125,37.5, GLOBAL_zombieFont, 30 )
...
local function onKeyEvent( event )
	...
	if event.keyName == "w" and event.phase == "down" then
		if selectedObject > table.getn(objectFileName)-1 then
			selectedObject = 1
		end
		if selectedObject > 1 then
			selectedObject = selectedObject -1
		else
			selectedObject = table.getn(objectFileName)-1
		end
	end
