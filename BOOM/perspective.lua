...
lib_perspective.createView = function(layerCount)
	...
	function view:setBounds(x1, x2, y1, y2)
		...
		if x1 then if not x1 then xMin = math_nhuge else xMin = x1 end end
		if x2 then if not x2 then xMax = math_huge else xMax = x2 end end
		if y1 then if not y1 then yMin = math_nhuge else yMin = y1 end end
		if y2 then if not y2 then yMax = math_huge else yMax = y2 end end
