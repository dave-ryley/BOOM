local S = {}
	...
	local function spawn(x, y)
		...
		local T = {}
			T.bounds = display.newImageRect( GLOBAL_graphicsPath.."/Items/ShotgunPowerUp.png", 100, 100 )
			...
			T.powerup = function(event)
				local fx = audio.loadSound( GLOBAL_soundsPath.."/PowerUps/ShotgunPowerUp.ogg")
				local other = event.other.super
				audio.play(fx,{channel = audio.findFreeChannel()})
				audio.dispose( fx )
				other.shotgun.powerUp(1)
				GLOBAL_shotgun = other.shotgun.power
