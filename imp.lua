...
local function spawn(startX, startY)
	...
	i.AI = function()
		if(i) then
			if(GLOBAL_pause == false and i.isDead == false) then
				if(i.hasTarget == true or i.shooting > 0) then
					if i.shooting == 0 then
						i.shooting = 1
						if(firstSight == true)then
							firstSight = false
							local laughfx = audio.loadSound(GLOBAL_soundsPath.."Enemies/ImpLaugh.ogg")
							audio.play(laughfx,{channel = audio.findFreeChannel()})
							audio.dispose( laughfx )
						end
						local impFire = audio.loadSound(GLOBAL_soundsPath.."Enemies/ImpFire.ogg" )
